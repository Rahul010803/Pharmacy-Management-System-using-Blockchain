...
def edit_medicines(medicine_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            sql_query = '''
                UPDATE medicine 
                SET medicine_name = %s, company = %s, 
                single_pack_quantity = %s, category_name = %s, location_rack = %s 
                WHERE medicine_id = %s
            '''
            values = (medicine_name, company, pack_detail, category, location_rack, medicine_id)
...
def display_categories():
    ...
    # categories = [dict(category_id=row[0], category_name=row[1]) for row in cursor.fetchall()]
    cursor.close()
    connection.close()
...
def edit_category_display(category_id):
    ...
    category = {'category_id': category_data[0], 'category_name': category_data[1]}
    ...
    # If the request method is GET, simply render the add_category_display.html template
    return render_template('edit_category_display.html')
...
def edit_location_rack(rack_id):
    print("Rack ID:", rack_id)  # Debugging: Print rack_id
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM location_rack WHERE rack_id = %s', (rack_id,))
    location_rack = cur.fetchall()
    print(location_rack)

    if not location_rack:
        flash('Rack not found', 'error')
        print("Rack not found for rack_id:", rack_id)  
        return redirect(url_for('location_rack'))
    ...
    if request.method == 'POST':
        ...
        if not rack_name:
            error_message = 'Rack Name cannot be empty'
            return render_template('edit_location_rack.html', error=error_message, location_rack=location_rack)
    ...
    return render_template('edit_location_rack.html', location_rack=location_rack)
